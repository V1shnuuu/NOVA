"""
JARVIS WorkMode — WiFi Presence Detection
Detects phone on local network via ICMP ping or ARP table scan.
"""

from __future__ import annotations

import re
import subprocess
import threading
import time
from collections import deque
from collections.abc import Callable

from utils.logger import get_logger

logger = get_logger("jarvis.triggers.wifi")


def ping_device(ip: str, timeout: int = 1) -> bool:
    """Send a single ICMP ping and return ``True`` if the device responds.

    Args:
        ip: Target IP address.
        timeout: Timeout in seconds.
    """
    try:
        result = subprocess.run(
            ["ping", "-n", "1", "-w", str(timeout * 1000), ip],
            capture_output=True,
            text=True,
            timeout=timeout + 2,
            creationflags=subprocess.CREATE_NO_WINDOW,
        )
        return "TTL=" in result.stdout.upper()
    except subprocess.TimeoutExpired:
        logger.debug(f"Ping to {ip} timed out.")
        return False
    except FileNotFoundError:
        logger.error("ping command not found on this system.")
        return False


def arp_scan(mac: str) -> bool:
    """Check the local ARP table for a given MAC address.

    Args:
        mac: Target MAC address (any common format).
    """
    normalised = mac.lower().replace(":", "-")
    try:
        result = subprocess.run(
            ["arp", "-a"],
            capture_output=True,
            text=True,
            timeout=5,
            creationflags=subprocess.CREATE_NO_WINDOW,
        )
        # Windows ARP output uses dashes: aa-bb-cc-dd-ee-ff
        return normalised in result.stdout.lower()
    except subprocess.TimeoutExpired:
        logger.debug("ARP scan timed out.")
        return False
    except FileNotFoundError:
        logger.error("arp command not found on this system.")
        return False


class WiFiPresenceDetector:
    """Background thread that polls for phone presence on the local network.

    Supports two strategies selectable via ``config.PRESENCE_DETECTION_METHOD``:
    * ``"ping"`` — ICMP echo request.
    * ``"arp"``  — ARP table lookup (more reliable when phone blocks ICMP).

    A "confirmed presence" requires ``PRESENCE_CONFIRM_COUNT`` detections
    within a rolling window of ``PRESENCE_WINDOW`` scans.
    """

    def __init__(self, config, callback: Callable[[str], None]) -> None:
        self.config = config
        self.callback = callback
        self._stop_event = threading.Event()
        self._thread: threading.Thread | None = None

    def start(self) -> None:
        """Spin up the background scanning thread."""
        self._stop_event.clear()
        self._thread = threading.Thread(
            target=self._scan_loop,
            name="WiFiPresenceScanner",
            daemon=True,
        )
        self._thread.start()
        logger.info(
            f"WiFi presence scanner started "
            f"(method={self.config.PRESENCE_DETECTION_METHOD}, "
            f"interval={self.config.WIFI_SCAN_INTERVAL}s)."
        )

    def stop(self) -> None:
        """Signal the scanning thread to stop and wait for it."""
        self._stop_event.set()
        if self._thread and self._thread.is_alive():
            self._thread.join(timeout=5)
        logger.info("WiFi presence scanner stopped.")

    # ── Private ──────────────────────────────────────────────────────

    def _scan_loop(self) -> None:
        """Core polling loop running in its own thread."""
        history: deque[bool] = deque(
            maxlen=self.config.PRESENCE_WINDOW,
        )

        while not self._stop_event.is_set():
            detected = self._detect()
            history.append(detected)
            logger.debug(
                f"Scan result: {'FOUND' if detected else 'NOT FOUND'} "
                f"(history={list(history)})"
            )

            hits = sum(history)
            if hits >= self.config.PRESENCE_CONFIRM_COUNT:
                logger.info(
                    f"Phone presence confirmed "
                    f"({hits}/{len(history)} scans positive)."
                )
                self.callback("wifi")
                # Clear history after triggering to avoid immediate re-fire
                history.clear()

            # Sleep in small increments so stop_event is responsive
            for _ in range(self.config.WIFI_SCAN_INTERVAL * 2):
                if self._stop_event.is_set():
                    return
                time.sleep(0.5)

    def _detect(self) -> bool:
        """Run the configured detection strategy."""
        method = self.config.PRESENCE_DETECTION_METHOD.lower()
        match method:
            case "ping":
                return ping_device(
                    self.config.PHONE_IP,
                    self.config.PHONE_PING_TIMEOUT,
                )
            case "arp":
                return arp_scan(self.config.PHONE_MAC)
            case _:
                logger.warning(
                    f"Unknown detection method '{method}'. "
                    f"Falling back to ARP."
                )
                return arp_scan(self.config.PHONE_MAC)
